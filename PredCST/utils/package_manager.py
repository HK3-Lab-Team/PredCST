...
def create_pyenv(package_name: str, version: Optional[str] = None) -> str:
    ...
    """
    env_name = (
        f"{package_name}_version_{version}_env" if version else f"{package_name}_env"
    )
...
def list_pip_versions(package_name: str) -> List[str]:
    ...
    result = subprocess.run(
        ["pip", "index", "versions", package_name], capture_output=True, text=True
    )

    # Check if the command was successful
    if result.returncode != 0:
        raise Exception(
            f"Failed to list versions for package '{package_name}'. Error: {result.stderr}"
        )
